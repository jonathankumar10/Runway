  ...
  return (
    <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-4 sm:p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-zinc-500">{total} recruiter{total !== 1 ? 's' : ''} tracked</span>
      </div>

      {total === 0 ? (
        <div className="flex flex-col items-center justify-center py-6 gap-2">
          <div className="w-9 h-9 rounded-full border border-dashed border-zinc-700 flex items-center justify-center">
            <Send size={15} className="text-zinc-600" />
          </div>
          <p className="text-xs text-zinc-500">No outreach tracked yet</p>
        </div>
      ) : (
        <div className="space-y-3">
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-xs text-zinc-400">Email sent</span>
              <span className="text-xs font-medium text-zinc-300">{emailCount} <span className="text-zinc-500">/ {total}</span></span>
            </div>
            <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
              <div className="h-full bg-blue-500 rounded-full transition-all duration-500" style={{ width: `${emailPct}%` }} />
            </div>
          </div>

          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-xs text-zinc-400">LinkedIn sent</span>
              <span className="text-xs font-medium text-zinc-300">{linkedInCount} <span className="text-zinc-500">/ {total}</span></span>
            </div>
            <div className="h-2 bg-zinc-800 rounded-full overflow-hidden">
          ...
          <div className="flex items-center gap-4 pt-2 border-t border-zinc-800 flex-wrap">
