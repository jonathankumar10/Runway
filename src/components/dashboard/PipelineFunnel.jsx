  ...
  return (
    <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-4 sm:p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-zinc-500">{totalActive} active</span>
      </div>
      <div className="space-y-2.5">
        {activeStageCounts.map(stage => (
          <div key={stage.id} className="flex items-center gap-3">
            <span className="text-xs text-zinc-400 w-28 shrink-0 truncate">{stage.label}</span>
            ...
            <span className="text-xs font-medium text-zinc-300 w-5 text-right">{stage.count}</span>
          </div>
        ))}
      </div>
    </div>
