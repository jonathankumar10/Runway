...
export default function PipelineFunnel() {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
      <h3 className="text-xs font-semibold text-slate-300 mb-4">Pipeline</h3>
      <div className="space-y-2">
        {activeStageCounts.map(stage => (
          <div key={stage.id} className="flex items-center gap-3">
            <span className="text-xs text-slate-500 w-28 shrink-0 truncate">{stage.label}</span>
            ...
            <span className="text-xs text-slate-400 w-5 text-right">{stage.count}</span>
          </div>
        ))}
      </div>
    </div>
